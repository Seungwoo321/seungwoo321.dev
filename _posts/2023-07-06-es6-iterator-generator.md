...
회사원 생성자 함수는 연봉정보를 저장하는 위크맵 생성하고 IIFE 함수로 감싼다. 그리고 외부에서는 setPay, getPay 메서드로만 연봉 정보에 접근할 수 있는 클래스를 반환한다.
...
셋(Set)은 중복을 허용하지 않는 데이터 집합니다. 그리고 위크셋(WeakSet)이 셋과 다른 점은 다음과 같다
...
이터레이터와 제너레이터와 함께 ES6에 도입된 매우 중요한 개념이다. 처음 도입된 개념이라서 이전의 자바스크립트에만 익숙하다면 생소할 수 있다.

이번에도 책의 예제 코드 그대로에 내용만 익숙한 애국가 1절로 변경해 보았다.
...
배열에 `values`메서드를 사용해서 이터레이터를 만들 수 있다. 그리고 next() 메서드를 호출하면 노래 한 구절을 부른 게 된다. 이 메서드는 지금 부른 노래 가사를 나타내는 `value`프로퍼티와 마지막까지 완창했는지를 나타내는 `done`프로퍼티를 가지는 객체를 반환한다. 더 진행할 것이 없으면 `done`은 `true` 가 되고 `value`는 `undefined` 가 된다. 중요한 점은 여기서 끝이 아니라  `next`를 계속 호출할 수 있다.
...
이터레이션 프로토콜은 모든 객체를 이터러블 객체로 바꿀 수 있다. 무슨 의미인지는 책에서 소개하는 다음 피보나치수열 예제 코드를 보면 이해할 수 있다.
...
책의 예제는 무지개색을 반환한다. 대신 추천 책 리스트를 담아봤다.
...
위의 이터레이터 파트에서 살펴본 것처럼 우선 이터레이터를 얻는다. 제너레이터를 호출하면 이터레이터를 얻을 수 있다. 다음 `next`를 호출해서 함수를 단계별로 진행할 수 있다.
...
위에서 작성한 예제와 같이 제너레이터를 활용하면 호출자가 함수의 실행을 제어할 수 있다. `yield`문은 제너레이터의 마지막 문이더라도 제너레이터를 끝내지 않는다. 그래서 예제와 같이  `return` 문을 사용하면 그 위치와 관계없이 `done` 은 `true` 가 되고 `value` 프로퍼티는 `return` 이 반환하는 값이 된다.
